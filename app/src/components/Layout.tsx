    ...
    <div className="min-h-screen" style={{ backgroundColor: '#F5F5F5' }}>
      ...
      <header className="sticky top-0 z-40" style={{ backgroundColor: UDISTRICT_COLORS.purple, borderBottom: `3px solid ${UDISTRICT_COLORS.gold}` }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 grid grid-cols-[minmax(0,auto)_1fr_auto] items-center gap-3">
          {/* Logo */}
          <NavLink to="/" className="-ml-1 sm:-ml-2 flex items-center gap-3 shrink-0 min-w-0 max-w-[320px] xl:max-w-[360px]" onClick={() => setMobileMenuOpen(false)}>
            <span
              aria-hidden="true"
          ...
          {/* Desktop Nav */}
          <nav className="hidden md:flex items-center justify-center gap-2 pl-4 lg:pl-6">
            {NAV_LINKS.map((link) => (
              <NavLink
                key={link.to}
                to={link.to}
