...
const VALUE_PROPS = [
  ...
  },
  {
    ...
    description: 'Our app verifies each student with their enrollment status, so you know every discount goes to a verified U-District student.',
  },
  {
    icon: Megaphone,
...
function MerchantCard({
  ...
  const frameStyle = isPurpleCard
    ? 'from-[#F4F0FF] to-[#EAE1FF] border-uw-purple/55'
    : 'from-[#FFF8E6] to-[#F6E2B8] border-uw-gold/65';

  return (
    <div
      className={`rounded-2xl border-[3px] p-6 transition-all duration-200 animate-card-enter shadow-[inset_0_0_0_1px_rgba(255,255,255,0.7)] bg-gradient-to-br ${frameStyle} ${
        isActive ? 'hover:-translate-y-0.5 hover:border-uw-purple/70' : 'hover:border-uw-gold/70'
      }`}
      style={{ animationDelay: `${index * 60}ms` }}
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5">
          <span className={`w-2 h-2 rounded-full ${isPurpleCard ? 'bg-uw-purple' : 'bg-uw-gold'}`} />
      ...
      {merchant.imageUrl && (
        <div className="mb-3 overflow-hidden rounded-xl border border-white/70 bg-white/60 shadow-sm">
          <img
            src={merchant.imageUrl}
            alt={`${merchant.name} photo`}
      ...
      <div className={`flex items-center gap-2 pt-3 border-t ${isActive ? 'border-[#E8E8E6]' : 'border-[#E8E8E6]/50'}`}>
        <Tag className={`w-3.5 h-3.5 shrink-0 ${isPurpleCard ? 'text-uw-purple' : 'text-uw-gold'}`} />
        <span className={`text-sm font-semibold ${isPurpleCard ? 'text-uw-purple' : 'text-uw-gold-dark'}`}>
          {merchant.discount}
        </span>
      </div>
      ...
      <a
        ...
        className={`w-full mt-3 flex items-center justify-center gap-2 h-9 rounded-full text-xs font-semibold transition-all active:scale-[0.97] ${
          isPurpleCard
            ...
            : 'bg-uw-gold text-uw-purple hover:bg-uw-gold-light'
        }`}
    ...
    <div>
      ...
      <section className="pt-10 pb-4 px-4 sm:px-6 max-w-3xl mx-auto text-center">
        ...
        <h1 className="text-[32px] sm:text-[48px] font-bold text-[#1A1A1A] leading-[1.15] tracking-tight mb-3">
          Grow Your Business with <span className="text-uw-purple">U-District Students.</span>
        </h1>
        <p className="text-sm sm:text-base text-[#6B6B6B] leading-relaxed max-w-lg mx-auto mb-6">
          Join the fastest-growing student marketplace on The Ave. List your business for free and connect with thousands of U-District students.
        </p>
        <button
          onClick={openPartner}
