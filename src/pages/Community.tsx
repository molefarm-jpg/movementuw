...
const TESTIMONIALS = [
  {
    name: 'Alex R.',
    role: 'UofW Junior',
    text: `I use the ${SITE_NAME} app almost every day. The discounts at One Bite Cafe and Mama Grande's are only available through the app — I have saved over $40 this quarter alone.`,
  },
  {
    name: 'Jordan M.',
    role: 'UofW Sophomore',
    text: `As a student on a budget, every dollar counts. The ${SITE_NAME} app is the only way to unlock these discounts. It makes supporting local businesses so easy.`,
  },
  {
    name: 'Priya S.',
    role: 'UofW Senior',
    text: `I love that I can just pull up the ${SITE_NAME} app and show my digital student ID. No fumbling for my UofW student ID — all the discounts are right there in one place.`,
  },
];
    ...
    <div>
      ...
      <section className="pt-10 pb-6 px-4 sm:px-6 max-w-3xl mx-auto text-center">
        ...
        <p className="text-sm sm:text-base text-[#6B6B6B] leading-relaxed max-w-lg mx-auto">
          {`The ${SITE_NAME} is more than discounts — it is a community of UofW students supporting local businesses through one shared app. Join thousands of UofW students already saving on The Ave. Download the app to be part of it.`}
        </p>
      </section>
          ...
          {[
            {
              icon: Heart,
              title: 'Support Local',
              desc: `Every discount redeemed through the ${SITE_NAME} app keeps dollars flowing into our neighborhood businesses.`,
            },
            {
              icon: Users,
              title: 'Student-Led',
              desc: `The ${SITE_NAME} app is built with student input at every step. Your voice shapes what we build.`,
            },
            {
              icon: MessageCircle,
              title: 'Spread the Word',
              desc: `Tell your friends, roommates, and classmates to download the ${SITE_NAME} app. The more students join, the more merchants we activate.`,
            },
          ].map((p) => (
            <div
              key={p.title}
